...
// Real-time command messages (cleared when leaving toolbox tab)
...
function createDefaultData() {
    return {
        ...
        // Manager directives (user instructions per stage)
        managerDirective: {
            global: '',
            extraction: '',
            recall: '',
...
function renderDirectiveTab() {
    const body = document.getElementById('mm_directive_panel_body');
    if (!body) return;

    const data = getMemoryData();
    const dir = data.managerDirective || { global: '', extraction: '', recall: '', compression: '' };

    body.innerHTML = `
        <div class="mm-directive-container">
            <details class="mm-directive-help">
                <summary style="cursor:pointer;font-size:12px;font-weight:600;color:var(--SmartThemeBodyColor,#999)">各环节说明 (点击展开)</summary>
    ...
    // Bind save
    body.querySelector('#mm_dir_save')?.addEventListener('click', () => {
        saveDirectives();
        toastr?.success?.('管理指令已保存');
    });

    // Bind clear
    body.querySelector('#mm_dir_clear')?.addEventListener('click', () => {
    ...
    // Bind organize
    body.querySelector('#mm_dir_organize')?.addEventListener('click', async () => {
        await organizeDirectives();
    });
}

function saveDirectives() {
    const data = getMemoryData();
    if (!data.managerDirective) data.managerDirective = {};
    data.managerDirective.global = document.getElementById('mm_dir_global')?.value?.trim() || '';
    data.managerDirective.extraction = document.getElementById('mm_dir_extraction')?.value?.trim() || '';
    data.managerDirective.recall = document.getElementById('mm_dir_recall')?.value?.trim() || '';
    data.managerDirective.compression = document.getElementById('mm_dir_compression')?.value?.trim() || '';
    saveMemoryData();
}

function clearDirectives() {
    const data = getMemoryData();
    data.managerDirective = { global: '', extraction: '', recall: '', compression: '' };
    saveMemoryData();
}

async function organizeDirectives() {
    // Read current textarea values first
    const global = document.getElementById('mm_dir_global')?.value?.trim() || '';
    const extraction = document.getElementById('mm_dir_extraction')?.value?.trim() || '';
    const recall = document.getElementById('mm_dir_recall')?.value?.trim() || '';
    const compression = document.getElementById('mm_dir_compression')?.value?.trim() || '';

    const combined = [global, extraction, recall, compression].filter(s => s).join('\n');
    if (!combined) {
        toastr?.warning?.('没有可整理的指令内容');
        return;
    }

    const organizeBtn = document.getElementById('mm_dir_organize');
    if (organizeBtn) { organizeBtn.disabled = true; organizeBtn.textContent = '整理中...'; }

    try {
        const systemPrompt = `你是一个记忆管理系统的助手。用户写了一些关于记忆管理各环节的指令，但可能比较混乱。
请帮用户整理成结构化的指令，分配到4个类别中。返回JSON格式：
{"global":"全局指令","extraction":"提取环节指令","recall":"召回环节指令","compression":"压缩环节指令"}
- global: 适用于所有环节的通用指令
- extraction: 专门针对记忆提取（写日记）环节的指令
- recall: 专门针对记忆召回（翻旧事）环节的指令
- compression: 专门针对页面压缩（浓缩摘要）环节的指令
保持用户原意，语言简洁，每个字段如果没有相关内容就留空字符串。`;

        const userPrompt = `当前用户的指令内容：
全局: ${global || '(空)'}
提取: ${extraction || '(空)'}
召回: ${recall || '(空)'}
压缩: ${compression || '(空)'}
...
请整理并返回JSON。`;
        ...
        const response = await callLLM(systemPrompt, userPrompt, 1024);
        const parsed = parseJsonResponse(response);
        if (parsed && typeof parsed === 'object') {
            const gEl = document.getElementById('mm_dir_global');
            const eEl = document.getElementById('mm_dir_extraction');
            const rEl = document.getElementById('mm_dir_recall');
            const cEl = document.getElementById('mm_dir_compression');
            if (gEl) gEl.value = parsed.global || '';
            if (eEl) eEl.value = parsed.extraction || '';
            if (rEl) rEl.value = parsed.recall || '';
            if (cEl) cEl.value = parsed.compression || '';
            toastr?.success?.('指令已整理，请检查后保存');
        } else {
            toastr?.warning?.('LLM返回格式异常，请手动整理');
        }
    } catch (err) {
        warn('Organize directives failed:', err);
        toastr?.error?.('整理失败: ' + err.message);
    } finally {
        if (organizeBtn) { organizeBtn.disabled = false; organizeBtn.textContent = '让管理员整理'; }
    }
}

/**
 * Get directive suffix to append to prompts for a given stage.
 * @param {'extraction'|'recall'|'compression'} stage
 * @returns {string} Empty string if no directives set
 */
function getDirectiveSuffix(stage) {
    const data = getMemoryData();
...
// ============================================================
//  Toolbox Tab (工具箱) — stub, will be fully implemented
// ============================================================

function renderToolboxTab() {
    const body = document.getElementById('mm_toolbox_panel_body');
    if (!body) return;
    ...
    body.innerHTML = `
        <div class="mm-toolbox-container">
            <div class="mm-toolbox-section">
                ...
                <small style="opacity:0.6">检查已删除消息对应的孤立记忆，建议清理。</small>
                <div class="mm-toolbox-row" style="margin-top:6px">
                    <button id="mm_health_check" class="mm-btn-cancel-entry">开始体检</button>
                </div>
                <div id="mm_health_results"></div>
            </div>
            <hr style="border-color:rgba(0,0,0,0.08);margin:8px 0" />
            <div class="mm-toolbox-section">
                <div class="mm-toolbox-section-title">快捷操作</div>
                <div class="mm-toolbox-row">
                    <label style="font-size:12px">提取范围:</label>
                    <input id="mm_quick_start" type="number" min="1" value="1" style="width:55px;font-size:12px;padding:2px 4px" />
                    <span style="font-size:12px">—</span>
                    <input id="mm_quick_end" type="number" min="1" value="10" style="width:55px;font-size:12px;padding:2px 4px" />
                    <button id="mm_quick_extract" class="mm-btn-cancel-entry" style="font-size:11px">提取</button>
                </div>
                <div class="mm-toolbox-row">
                    <label style="font-size:12px">标记已提取:</label>
                    <input id="mm_mark_start" type="number" min="1" value="1" style="width:55px;font-size:12px;padding:2px 4px" />
                    <span style="font-size:12px">—</span>
                    <input id="mm_mark_end" type="number" min="1" value="10" style="width:55px;font-size:12px;padding:2px 4px" />
                    <button id="mm_quick_mark" class="mm-btn-cancel-entry" style="font-size:11px">标记</button>
                </div>
                <div class="mm-toolbox-row">
                    <button id="mm_quick_rebuild_vectors" class="mm-btn-cancel-entry" style="font-size:11px">重建向量库</button>
                </div>
            </div>
            <hr style="border-color:rgba(0,0,0,0.08);margin:8px 0" />
            <div class="mm-toolbox-section">
                <div class="mm-toolbox-section-title">实时指令</div>
                <small style="opacity:0.6">和记忆管理员对话，直接操作记忆数据。离开此页面自动清除对话（省钱）。</small>
                <div id="mm_rt_messages" class="mm-rt-messages"></div>
                <div class="mm-rt-input-row">
                    <input id="mm_rt_input" type="text" class="text_pole" placeholder="输入指令..." style="flex:1;font-size:12px" />
                    <button id="mm_rt_send" class="mm-btn-save-entry" style="font-size:11px;padding:3px 10px">发送</button>
                </div>
                <div id="mm_rt_no_api" style="display:none;font-size:12px;color:#ef4444;margin-top:4px">
                    需要配置副API才能使用实时指令功能。
                </div>
            </div>
        </div>
    ...
    // Bind health check
    body.querySelector('#mm_health_check')?.addEventListener('click', runHealthCheck);

    // Bind quick actions
    body.querySelector('#mm_quick_extract')?.addEventListener('click', quickExtractRange);
    body.querySelector('#mm_quick_mark')?.addEventListener('click', quickMarkExtracted);
    body.querySelector('#mm_quick_rebuild_vectors')?.addEventListener('click', async () => {
        if (!confirm('重建向量库？')) return;
        try {
            toastr?.info?.('正在重建向量库...');
            await rebuildAllVectors();
            const data = getMemoryData();
            toastr?.success?.(`向量库重建完成，${Object.keys(data.embeddings).length} 页`);
        } catch (e) {
            toastr?.error?.('重建失败: ' + e.message);
        }
    });

    // Bind real-time command
    const s = getSettings();
    if (!s.useSecondaryApi) {
        const noApiEl = body.querySelector('#mm_rt_no_api');
        if (noApiEl) noApiEl.style.display = '';
    }
    body.querySelector('#mm_rt_send')?.addEventListener('click', sendRealTimeCommand);
    body.querySelector('#mm_rt_input')?.addEventListener('keydown', (e) => {
...
// ============================================================
//  Toolbox Functions (工具箱功能)
// ============================================================

function runHealthCheck() {
    const ctx = getContext();
    const data = getMemoryData();
    const resultsEl = document.getElementById('mm_health_results');
    if (!resultsEl) return;

    const dates = data.processing.extractedMsgDates || {};
    if (Object.keys(dates).length === 0) {
        resultsEl.innerHTML = '<div style="font-size:12px;margin-top:6px;color:var(--SmartThemeBodyColor,#999)">没有提取记录，无法进行体检。</div>';
        return;
    }

    // Build set of actual chat send_dates
    ...
    // Find orphaned dates (in extractedMsgDates but not in current chat)
    const orphanedDates = new Set();
    for (const d of Object.keys(dates)) {
        if (!chatDates.has(d)) orphanedDates.add(d);
    }

    if (orphanedDates.size === 0) {
        resultsEl.innerHTML = '<div style="font-size:12px;margin-top:6px;color:#22c55e">✓ 记忆健康，未发现孤立数据。</div>';
        return;
    }

    // Check which pages have orphaned sourceDates
    const affectedPages = [];
    for (const page of data.pages) {
        const sd = page.sourceDates || [];
        if (sd.length === 0) continue;
        const orphanCount = sd.filter(d => orphanedDates.has(d)).length;
        if (orphanCount === 0) continue;
        const allOrphaned = orphanCount === sd.length;
        affectedPages.push({
            page,
            orphanCount,
            total: sd.length,
    ...
    let html = `<div style="font-size:12px;margin-top:6px;color:#f59e0b">
        发现 ${orphanedDates.size} 条已提取但已不存在于聊天中的消息记录。
    </div>`;

    if (affectedPages.length > 0) {
        html += `<div style="font-size:12px;margin-top:4px">受影响的故事页：</div>`;
        html += '<div class="mm-health-list">';
        for (const item of affectedPages) {
            const statusText = item.allOrphaned
                ? '<span style="color:#ef4444">全部来源已删除 — 建议删除</span>'
                : `<span style="color:#f59e0b">${item.orphanCount}/${item.total} 来源已删除 — 建议审查</span>`;
            html += `<label class="mm-health-item">
                <input type="checkbox" value="${item.page.id}" ${item.allOrphaned ? 'checked' : ''} />
                <span>${escapeHtml(item.page.day)} ${escapeHtml(item.page.title)}</span>
                <span style="font-size:11px;margin-left:auto">${statusText}</span>
            </label>`;
        }
        html += '</div>';
        html += `<div class="mm-toolbox-row" style="margin-top:6px">
            <button id="mm_health_delete_selected" class="mm-btn-cancel-entry" style="color:#ef4444;font-size:11px">删除选中</button>
            <button id="mm_health_clean_dates" class="mm-btn-cancel-entry" style="font-size:11px">清理孤立日期</button>
        </div>`;
    } else {
        html += `<div style="font-size:12px;margin-top:4px;color:var(--SmartThemeBodyColor,#999)">
            没有故事页受到影响（可能页面没有 sourceDates 记录）。
        </div>`;
        html += `<div class="mm-toolbox-row" style="margin-top:6px">
            <button id="mm_health_clean_dates" class="mm-btn-cancel-entry" style="font-size:11px">清理孤立日期</button>
        </div>`;
    ...
    resultsEl.querySelector('#mm_health_delete_selected')?.addEventListener('click', () => {
        const checked = resultsEl.querySelectorAll('.mm-health-item input:checked');
        if (checked.length === 0) { toastr?.warning?.('请先选中要删除的故事页'); return; }
        if (!confirm(`确认删除 ${checked.length} 个故事页？`)) return;
        ...
        saveMemoryData();
        updateBrowserUI();
        toastr?.success?.(`已删除 ${checked.length} 个故事页`);
        runHealthCheck(); // Re-run
    });

    // Bind clean orphaned dates
    resultsEl.querySelector('#mm_health_clean_dates')?.addEventListener('click', () => {
        let cleaned = 0;
        for (const d of orphanedDates) {
            delete dates[d];
            cleaned++;
        }
        saveMemoryData();
...
async function quickExtractRange() {
    const startEl = document.getElementById('mm_quick_start');
    const endEl = document.getElementById('mm_quick_end');
    ...
    // Collect messages in range (1-indexed)
    const messages = [];
    const sourceDates = [];
    let msgIndex = 0;
    for (let i = 0; i < ctx.chat.length; i++) {
        ...
        if (msgIndex >= start && msgIndex <= end) {
            messages.push(m);
            if (m.send_date) sourceDates.push(m.send_date);
        }
    }

    if (messages.length === 0) {
        toastr?.warning?.('指定范围内没有找到消息');
        return;
    }

    if (!confirm(`将提取第 ${start} ~ ${end} 条消息（共 ${messages.length} 条），确认？`)) return;

    toastr?.info?.(`正在提取 ${messages.length} 条消息...`);
    setMood('thinking');
    ...
    try {
        ...
        // Build batch text
        const batchText = messages.map(m => {
            const role = m.is_user ? userName : charName;
            return `[${role}]: ${m.mes}`;
        }).join('\n');
        ...
        const response = await callLLM(
            '你是剧情记忆管理系统。严格按要求输出JSON。',
            prompt,
            s.extractionMaxTokens || 4096,
        );
        const result = parseJsonResponse(response);

        if (result) {
            applyExtractionResult(data, result, sourceDates);
            // Mark these messages as extracted
            for (const d of sourceDates) {
                data.processing.extractedMsgDates[d] = true;
            }
            saveMemoryData();
            updateBrowserUI();
            toastr?.success?.(`提取完成，范围 ${start}-${end}`);
        } else {
            toastr?.error?.('提取结果解析失败');
        }
    } catch (err) {
        warn('Quick extract range failed:', err);
...
function quickMarkExtracted() {
    const startEl = document.getElementById('mm_mark_start');
    const endEl = document.getElementById('mm_mark_end');
    if (!startEl || !endEl) return;
    ...
    for (let i = 0; i < ctx.chat.length; i++) {
        ...
        if (!m || !m.mes) continue;
        msgIndex++;
        if (msgIndex >= start && msgIndex <= end && m.send_date) {
            data.processing.extractedMsgDates[m.send_date] = true;
            marked++;
        }
    }
    ...
    if (marked > 0) {
        ...
        data.processing.lastExtractedMessageId = Math.max(
            data.processing.lastExtractedMessageId,
            end
        );
        saveMemoryData();
        updateBrowserUI();
        toastr?.success?.(`已标记 ${marked} 条消息为已提取`);
    } else {
        toastr?.warning?.('指定范围内没有找到可标记的消息');
    }
}

// ── Real-time commands (chat-style agent) ──

function buildRealTimeAgentTools(data) {
    return [
        {
            type: 'function',
            function: {
                name: 'search_pages',
                description: '搜索故事页。可按关键词、分类、天数范围筛选。',
                parameters: {
                    type: 'object',
                    properties: {
                        keyword: { type: 'string', description: '搜索关键词（匹配标题、内容、关键词）' },
                        category: { type: 'string', description: '分类标签筛选', enum: Object.keys(MEMORY_CATEGORIES) },
                        day_start: { type: 'string', description: '起始天数，如 D1' },
                        day_end: { type: 'string', description: '结束天数，如 D5' },
                    },
                },
            },
        },
        {
            ...
            function: {
                ...
                description: '编辑故事页的某个字段。可修改 title/day/date/content/keywords/categories/significance。',
                parameters: {
                    type: 'object',
                    properties: {
                        page_id: { type: 'string', description: '故事页ID' },
                        field: { type: 'string', description: '字段名', enum: ['title', 'day', 'date', 'content', 'keywords', 'categories', 'significance'] },
                        value: { type: 'string', description: '新值（keywords/categories用逗号分隔）' },
                    },
        ...
        },
        {
            ...
            function: {
                name: 'extract_range',
                description: '提取指定范围的消息为记忆。start和end为消息编号（1开始）。',
                parameters: {
                    type: 'object',
                    properties: {
                        start: { type: 'number', description: '起始消息编号' },
                        end: { type: 'number', description: '结束消息编号' },
                    },
                    required: ['start', 'end'],
                },
            },
        },
        {
            ...
            function: {
                name: 'mark_extracted',
                description: '标记指定范围的消息为已提取（不实际提取，只标记）。',
                parameters: {
                    type: 'object',
                    properties: {
                        start: { type: 'number', description: '起始消息编号' },
...
async function executeRealTimeTool(name, args, data) {
    switch (name) {
        case 'search_pages': {
            ...
            if (args.category) {
                results = results.filter(p => (p.categories || []).includes(args.category));
            }
            if (args.day_start || args.day_end) {
                const parseDay = d => parseInt((d || '').replace(/\D/g, '') || '0');
                const ds = parseDay(args.day_start) || 0;
                const de = parseDay(args.day_end) || 99999;
                results = results.filter(p => {
                    const pd = parseDay(p.day);
                    return pd >= ds && pd <= de;
                });
            }
            return JSON.stringify(results.map(p => ({
                id: p.id, day: p.day, title: p.title, content: p.content.substring(0, 100),
                categories: p.categories, keywords: p.keywords, significance: p.significance,
            })));
        }
        case 'edit_page_field': {
            const page = data.pages.find(p => p.id === args.page_id);
            if (!page) return JSON.stringify({ error: '未找到页面: ' + args.page_id });
            const field = args.field;
            const value = args.value;
            if (field === 'keywords') {
                page.keywords = value.split(/[,，]/).map(k => k.trim()).filter(k => k);
            } else if (field === 'categories') {
                page.categories = value.split(/[,，]/).map(c => c.trim()).filter(c => VALID_CATEGORIES.has(c));
            } else if (['title', 'day', 'date', 'content', 'significance'].includes(field)) {
                page[field] = value;
            } else {
                return JSON.stringify({ error: '不支持的字段: ' + field });
            }
            saveMemoryData();
            return JSON.stringify({ success: true, page_id: args.page_id, field, value: page[field] });
        }
        case 'delete_page': {
            const idx = data.pages.findIndex(p => p.id === args.page_id);
            if (idx === -1) return JSON.stringify({ error: '未找到页面: ' + args.page_id });
            const removed = data.pages.splice(idx, 1)[0];
            if (data.embeddings) delete data.embeddings[args.page_id];
            saveMemoryData();
            return JSON.stringify({ success: true, deleted: removed.title });
        }
        case 'extract_range': {
            // Use quick extract logic
            const startEl = document.getElementById('mm_quick_start');
            const endEl = document.getElementById('mm_quick_end');
            if (startEl) startEl.value = args.start;
            if (endEl) endEl.value = args.end;
            await quickExtractRange();
            return JSON.stringify({ success: true, range: `${args.start}-${args.end}` });
        }
        case 'mark_extracted': {
            const startEl = document.getElementById('mm_mark_start');
            const endEl = document.getElementById('mm_mark_end');
        ...
        case 'compress_page': {
            try {
                await compressPage(data, args.page_id);
                saveMemoryData();
                return JSON.stringify({ success: true, page_id: args.page_id });
            } catch (e) {
        ...
        case 'rebuild_embeddings': {
            try {
                await rebuildAllVectors();
                return JSON.stringify({ success: true, count: Object.keys(data.embeddings).length });
            } catch (e) {
                return JSON.stringify({ error: e.message });
            }
        }
...
async function sendRealTimeCommand() {
    const input = document.getElementById('mm_rt_input');
    const messagesEl = document.getElementById('mm_rt_messages');
    ...
    const s = getSettings();
    if (!s.useSecondaryApi) {
        toastr?.warning?.('请先配置副API');
        return;
    }

    // Add user message to display
    messagesEl.innerHTML += `<div class="mm-rt-msg mm-rt-user">${escapeHtml(text)}</div>`;
    ...
    const systemMsg = {
        role: 'system',
        content: `你是记忆管理助手。用户会用自然语言给你指令来操作记忆数据。
当前记忆概况：${data.pages.length} 个故事页，时间线${data.timeline ? '有数据' : '为空'}。
请使用提供的工具完成用户的要求。操作完成后简要回复结果。`,
    };

    rtCommandMessages.push({ role: 'user', content: text });

    const tools = buildRealTimeAgentTools(data);
    const allMessages = [systemMsg, ...rtCommandMessages];

    // Show thinking indicator
    const thinkingId = 'mm_rt_thinking_' + Date.now();
    messagesEl.innerHTML += `<div id="${thinkingId}" class="mm-rt-msg mm-rt-assistant" style="opacity:0.5">思考中...</div>`;
    messagesEl.scrollTop = messagesEl.scrollHeight;
    setMood('thinking');
    ...
    try {
        ...
        while (rounds < maxRounds) {
            rounds++;
            const response = await callSecondaryApiChat(currentMessages, tools, 2048);

            if (response.rawToolCalls && response.rawToolCalls.length > 0) {
                // Execute tools — use rawToolCalls for tc.id and tc.function format
                ...
                for (const tc of response.rawToolCalls) {
                    ...
                    const toolResult = await executeRealTimeTool(tc.function?.name || '', args, data);
                ...
                rtCommandMessages.push({ role: 'assistant', content: reply });
                // Remove thinking indicator and add real reply
                const thinkingEl = document.getElementById(thinkingId);
                if (thinkingEl) thinkingEl.remove();
                messagesEl.innerHTML += `<div class="mm-rt-msg mm-rt-assistant">${escapeHtml(reply)}</div>`;
                messagesEl.scrollTop = messagesEl.scrollHeight;
                updateBrowserUI();
                break;
            }
        ...
        if (rounds >= maxRounds) {
            const thinkingEl = document.getElementById(thinkingId);
            if (thinkingEl) thinkingEl.remove();
            messagesEl.innerHTML += `<div class="mm-rt-msg mm-rt-assistant" style="color:#f59e0b">已达最大轮次限制，操作可能未完全完成。</div>`;
            messagesEl.scrollTop = messagesEl.scrollHeight;
        }
    } catch (err) {
        ...
        const thinkingEl = document.getElementById(thinkingId);
        if (thinkingEl) thinkingEl.remove();
        messagesEl.innerHTML += `<div class="mm-rt-msg mm-rt-assistant" style="color:#ef4444">错误: ${escapeHtml(err.message)}</div>`;
        messagesEl.scrollTop = messagesEl.scrollHeight;
    } finally {
        setMood('idle');
    }
...
function bindRecallFab() {
    ...
    panel.innerHTML = `
        ...
        <div class="mm-recall-tab-content mm-recall-tab-active" data-tab="recall">
            <div id="mm_recall_panel_body" class="mm-recall-panel-body">
                <div class="mm-empty-state">尚无召回内容</div>
            </div>
        </div>
        <div class="mm-recall-tab-content" data-tab="directive" style="display:none">
            <div id="mm_directive_panel_body" class="mm-recall-panel-body">
                <div class="mm-empty-state">加载中...</div>
            </div>
        </div>
        <div class="mm-recall-tab-content" data-tab="toolbox" style="display:none">
            <div id="mm_toolbox_panel_body" class="mm-recall-panel-body">
                <div class="mm-empty-state">加载中...</div>
            </div>
        </div>
    `;
    ...
    // ── Tab switching logic ──
    let directiveInitialized = false;
    let toolboxInitialized = false;

    panel.querySelectorAll('.mm-recall-tabs .mm-recall-tab').forEach(tabBtn => {
        tabBtn.addEventListener('click', () => {
            const targetTab = tabBtn.dataset.tab;

            // Deactivate all tabs and content
            panel.querySelectorAll('.mm-recall-tab').forEach(t => t.classList.remove('mm-recall-tab-active'));
            panel.querySelectorAll('.mm-recall-tab-content').forEach(c => {
                c.classList.remove('mm-recall-tab-active');
                c.style.display = 'none';
            });

            // Activate target
            tabBtn.classList.add('mm-recall-tab-active');
            const targetContent = panel.querySelector(`.mm-recall-tab-content[data-tab="${targetTab}"]`);
            if (targetContent) {
                targetContent.classList.add('mm-recall-tab-active');
                targetContent.style.display = '';
            }

            // Lazy-init tabs
            if (targetTab === 'directive' && !directiveInitialized) {
                directiveInitialized = true;
                renderDirectiveTab();
            }
            if (targetTab === 'toolbox' && !toolboxInitialized) {
                toolboxInitialized = true;
                renderToolboxTab();
            }

            // Clear real-time command context when leaving toolbox (save money)
            if (targetTab !== 'toolbox') {
                rtCommandMessages = [];
            }
        });
    });
        ...
        if (allPages.length === 0) {
            ...
            listEl.innerHTML = allPages.map(p => {
                ...
                <div class="mm-memory-card ${levelClass}" data-id="${p.id}">
                    ...
                    <div class="mm-page-edit-panel" style="display:none"></div>
                </div>
            `}).join('');
...
function onAddPage() {
    ...
    saveMemoryData();
    updateBrowserUI();
    // Auto-open edit form for the new page
    setTimeout(() => onEditPage(newId), 50);
}
...
async function onEditPage(id) {
    ...
    const headerEl = card.querySelector('.mm-memory-card-header');
    const tagsEl = card.querySelector('.mm-memory-card-tags');
    const bodyEl = card.querySelector('.mm-memory-card-body');
    const actionsEl = card.querySelector('.mm-memory-card-actions');
    const editPanel = card.querySelector('.mm-page-edit-panel');
    if (!editPanel) return;
    ...
    const catCheckboxes = Object.entries(MEMORY_CATEGORIES).map(([key, label]) => {
        ...
        return `<label class="mm-cat-checkbox" style="color:${color}">
            <input type="checkbox" value="${key}" ${checked} /> ${escapeHtml(label)}
        </label>`;
    }).join('');
    ...
    editPanel.innerHTML = `
        <div class="mm-page-edit-form">
            <div class="mm-edit-field">
                <label>标题</label>
                <input class="text_pole mm-pf-title" type="text" value="${escapeHtml(page.title || '')}" />
            </div>
            <div class="mm-edit-field-row">
                <div class="mm-edit-field" style="flex:1">
                    <label>天数 (如 D1)</label>
                    <input class="text_pole mm-pf-day" type="text" value="${escapeHtml(page.day || '')}" placeholder="D1" />
                </div>
                <div class="mm-edit-field" style="flex:1">
                    <label>日期 (如 251017)</label>
                    <input class="text_pole mm-pf-date" type="text" value="${escapeHtml(page.date || '')}" placeholder="251017" />
                </div>
            </div>
            <div class="mm-edit-field">
                <label>内容</label>
                <textarea class="text_pole mm-pf-content" rows="4">${escapeHtml(page.content || '')}</textarea>
            </div>
            <div class="mm-edit-field">
                <label>关键词 (逗号分隔)</label>
                <input class="text_pole mm-pf-keywords" type="text" value="${escapeHtml((page.keywords || []).join(', '))}" placeholder="关键词1, 关键词2" />
            </div>
            <div class="mm-edit-field">
                <label>分类标签</label>
            ...
            <div class="mm-edit-field">
                <label>重要程度</label>
                <select class="text_pole mm-pf-significance">
                    <option value="high" ${page.significance === 'high' ? 'selected' : ''}>高 (!!)</option>
                    <option value="medium" ${page.significance !== 'high' ? 'selected' : ''}>中 (!)</option>
                </select>
            </div>
    ...
    // Save handler
    editPanel.querySelector('.mm-btn-save-entry').addEventListener('click', async () => {
        page.title = editPanel.querySelector('.mm-pf-title').value.trim() || page.title;
        page.day = editPanel.querySelector('.mm-pf-day').value.trim();
        page.date = editPanel.querySelector('.mm-pf-date').value.trim();
        page.content = editPanel.querySelector('.mm-pf-content').value.trim();
        page.keywords = editPanel.querySelector('.mm-pf-keywords').value
            .split(/[,，]/)
            .map(k => k.trim())
            .filter(k => k.length > 0);
        page.categories = Array.from(editPanel.querySelectorAll('.mm-cat-checkboxes input:checked'))
            .map(cb => cb.value)
            .filter(v => VALID_CATEGORIES.has(v));
        page.significance = editPanel.querySelector('.mm-pf-significance').value;
        saveMemoryData();
        // Re-embed if embedding is enabled
        const s = getSettings();
        if (s.useEmbedding) {
            try { await embedPage(page); } catch (e) { warn('Re-embed after edit failed:', e); }
        }
        updateBrowserUI();
    });

    // Cancel handler
    editPanel.querySelector('.mm-btn-cancel-entry').addEventListener('click', () => {
        updateBrowserUI();
    });

    editPanel.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    editPanel.querySelector('.mm-pf-title')?.focus();
}
